...
binary's reachability of exit(1) is what monster/Z3 tests.
...
def _build_harness(func_name: str, params: list[tuple[str, str]]) -> str:
    ...
    Build a C* main() harness that:
      ...
      4. The function itself will call exit(1) if a violation occurs.
         The harness calls exit(0) if the function completes normally.
    ...
    stmts.append(f"  {func_name}({args});")
    stmts.append(f"  exit(0);")
...
def inject_check(source: str, check_statement: str) -> str:
    ...
    The harness calls exit(1) (not return 1) when the violation fires,
    so that monster's SMT exit-condition encoding is correct.

    Args:
        source          : full C* source code (must contain exactly one
                          non-main function with a 'return result;')
        check_statement : e.g. "if (result < 0) { exit(1); }"

    Returns:
        Patched C* source ready to be compiled by monster.
    ...
    func_name, params = parse_signature(source)
